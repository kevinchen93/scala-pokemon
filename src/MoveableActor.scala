 ...
 */
abstract class MoveableActor(x: Int, y: Int, var name: String) extends Actor(x, y) {

  def moveUp() = y -= 1
  def moveDown() = y += 1
  def moveLeft() = x -= 1
  def moveRight() = x += 1
  def adjacentTo(other: Player) = (sameX(other) && nextToY(other)) || (sameY(other) && nextToX(other))
