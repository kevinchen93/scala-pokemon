public class Attacks {
    ...
    public int getDamage(Monsters enemy) {
    	if (move == "Growl") {
    		if (enemy.curAttack <= 0) {
    			System.out.println("It had no effect!");
    		}
    		else {
    			enemy.curAttack = enemy.curAttack - 1;
    			System.out.println(enemy.name + "'s attack has been lowered to " + enemy.curAttack + " / " + enemy.attack);
    		}
    		return 0;
    	}
    	if (move == "Tail Whip") {
    		if (enemy.curDef <= 0) {
	    		...
	    		enemy.curDef = enemy.curDef - 1;
	    		System.out.println(enemy.name + "'s defense has been lowered to " + enemy.curDef + " / " + enemy.defense);
    		}
    		return 0;
    	}
    	if (move == "Thunderwave") {
